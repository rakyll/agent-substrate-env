...
// newServeCmd builds the "serve" subcommand: the control plane that drives
// actor lifecycle (create, resume, suspend) via the Agent Substrate control
// API. Tool execution lives in the "actor" subcommand.
func newServeCmd() *cobra.Command {
	var configPath string
	cmd := &cobra.Command{
		Use:   "serve",
		Short: "Run the control plane (actor lifecycle: resume, suspend)",
		Args:  cobra.NoArgs,
...
func runServe(configPath string) error {
	...
	log.Printf("Starting Agent Substrate environment control plane...")
	...
	//
	// Lifecycle transitions hang off the session resource as trailing action
	// segments (rather than the AIP-style {id}:resume custom method, since
	// net/http requires a path wildcard to span a full segment).
